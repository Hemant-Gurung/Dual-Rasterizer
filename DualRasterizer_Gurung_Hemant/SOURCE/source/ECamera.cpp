...
namespace Elite
{
	...
	void Camera::Update(float elapsedSec, float Swidth, float Sheight)
	{
		//aspect ratio
		m_AspectRatio = Swidth / Sheight;

		CalculateProjectionMatrix();
		CalculateProjectionMatrixDirectX();
	...
	void Camera::CalculateLookAt()
	{
		...
		m_ViewToWorldHardware =
		{
			//flip axis in hardware
			FVector4{-xAxis},
			FVector4{yAxis},
			FVector4{zAxis},
		...
		m_WorldToViewHardware = Inverse(m_ViewToWorldHardware);

		m_ViewToWorldSoftware =
		{
			//flip xaxis for front side
			FVector4{-xAxis},
			FVector4{yAxis},
			//flip z axis for similar rotation with hardware
			FVector4{-zAxis},
			FVector4{m_Position.x,m_Position.y,m_Position.z,1.f}
		};
		//Construct World2View Matrix
		//m_WorldToViewSoftware = Inverse(m_ViewToWorld);
		m_WorldToViewSoftware = Inverse(m_ViewToWorldSoftware );
	}
	void Camera::CalculateProjectionMatrix()
	{
		...
		Elite::FVector4{ 0,0, m_Fplane / (m_Fplane - m_Nplane), 1},
		Elite::FVector4{ 0, 0,-(m_Fplane * m_Nplane) / (m_Fplane - m_Nplane),0 }
		};


	}

	void Camera::CalculateProjectionMatrixDirectX()
	{
		m_ProjectionMatrixRightHS = Elite::FMatrix4{
		Elite::FVector4{ 1 / (m_AspectRatio * m_Fov),0,0, 0 },
		Elite::FVector4{ 0,                1 / m_Fov,0, 0 },
		Elite::FVector4{ 0,0, -m_Fplane / (m_Fplane - m_Nplane), -1},
		Elite::FVector4{ 0, 0,-(m_Fplane * m_Nplane) / (m_Fplane - m_Nplane),0 }
	...
	void Camera::SetDirectXRasterizationActive(bool isActive)
	{
		m_isSoftwareOn = isActive;
	}
